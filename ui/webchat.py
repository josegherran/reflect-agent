...
# Simple webchat app for testing the ReFlectPromptingAgent
...
async def chat(request: Request):
	...
	# For demo, use the agent's PromptTemplateSkill for prompt generation
	if user_msg.lower().startswith("evaluate"):
		reply = agent.skills[0].execute(user_msg)
	else:
		reply = agent.skills[1].execute(task=user_msg)
